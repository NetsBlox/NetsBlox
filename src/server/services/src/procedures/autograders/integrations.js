...
class CourseraIntegration {
    ...
    validate(config) {
        config.assignments.forEach(assignment => {
            assert(assignment.CourseraAssignmentKey, new CourseraKeyError(assignment.name, 'CourseraAssignmentKey'));
            assignment.tests.forEach((test, index) => {
                const name = `Test #${index+1} for ${assignment.name}`;
                assert(test.CourseraPartID, new CourseraKeyError(name, 'CourseraPartID'));
            });
        });
    }
}
...
module.exports = {
    get: function (name) {
        ...
        throw new Error(`"${name}" is not a valid integration. Did you mean "coursera"?`);
    },
    errors: {
        CourseraKeyError,
    }
