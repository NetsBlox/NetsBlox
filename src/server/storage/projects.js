        ...
        setRoles(roles) {
            ...
            return Q.all(roles.map(role => storeRoleBlob(role)))
                .then(roles => {
                    ...
                    const ids = this.getRoleIds(roles);
                    // TODO: update this to use the ids...
                    roles.forEach((role, i) => query.$set[`roles.${ids[i]}`] = role);
