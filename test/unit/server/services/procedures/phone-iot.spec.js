const utils = require('../../../../assets/utils');

describe(utils.suiteName(__filename), function() {
    utils.verifyRPCInterfaces('PhoneIoT', [
        ['getDevices', []],
        ['getColor', ['red', 'green', 'blue']],
        ['authenticate', ['device']],
        ['clearControls', ['device']],
        ['removeControl', ['device', 'id']],
        ['addImageDisplay', ['device', 'x', 'y', 'width', 'height', 'id', 'event']],
        ['addButton', ['device', 'x', 'y', 'width', 'height', 'color', 'textColor', 'id', 'event', 'text']],
        ['addTextField', ['device', 'x', 'y', 'width', 'height', 'color', 'textColor', 'id', 'event', 'text']],
        ['addCheckbox', ['device', 'x', 'y', 'checkColor', 'textColor', 'state', 'id', 'event', 'text']],
        ['addToggleswitch', ['device', 'x', 'y', 'checkColor', 'textColor', 'state', 'id', 'event', 'text']],
        ['addRadioButton', ['device', 'x', 'y', 'checkColor', 'textColor', 'state', 'id', 'group', 'event', 'text']],
        ['addLabel', ['device', 'x', 'y', 'textColor', 'id', 'text']],
        ['listen', ['device']],
        ['getToggleState', ['device', 'id']],
        ['getOrientation', ['device']],
        ['getCompassHeading', ['device']],
        ['getCompassDirection', ['device']],
        ['getCompassCardinalDirection', ['device']],
        ['getAccelerometer', ['device']],
        ['getFacingDirection', ['device']],
        ['getGravity', ['device']],
        ['getLinearAcceleration', ['device']],
        ['getGyroscope', ['device']],
        ['getRotation', ['device']],
        ['getGameRotation', ['device']],
        ['getMagneticFieldVector', ['device']],
        ['getMicrophoneLevel', ['device']],
        ['getLocation', ['device']],
        ['getProximity', ['device']],
        ['getBearing', ['device']],
        ['getAltitude', ['device']],
        ['getStepCount', ['device']],
        ['getLightLevel', ['device']],
        ['getImage', ['device', 'id']],
        ['setImage', ['device', 'id', 'img']],
        ['magnitude', ['vec']],
        ['normalize', ['vec']],
        ['setText', ['device', 'id', 'text']],
        ['getText', ['device', 'id']],
        ['setCredentials', ['device', 'password']],
    ]);
});
